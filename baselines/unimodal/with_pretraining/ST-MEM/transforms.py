...
class Resample:
    def __init__(self, target_fs=250, target_length=2250):
...
class RandomCrop:
    """Randomly crop a segment of the input sequence."""
    def __init__(self, crop_length):
        self.crop_length = crop_length

    def __call__(self, x):
        # Validate input length
        if x.shape[-1] < self.crop_length:
            print(f"Warning: Sequence length {x.shape[-1]} is smaller than crop length {self.crop_length}. Skipping.")
            return np.zeros((x.shape[0], self.crop_length), dtype=x.dtype)  # Pad with zeros instead of skipping

        start = np.random.randint(0, x.shape[-1] - self.crop_length + 1)
...
class CenterCrop:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        crop_length = min(self.crop_length, x.shape[1])
        start_idx = (x.shape[1] - crop_length) // 2
        return x[:, start_idx:start_idx + crop_length]
...
class NCrop:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.num_segments > 1:
            step = max(1, (x.shape[1] - crop_length) // (self.num_segments - 1))
        else:
            step = 1  # Ensure step is never zero
        start_idx = np.arange(0, x.shape[1] - crop_length + 1, step)
        return np.stack([x[:, i:i + crop_length] for i in start_idx], axis=0)
...
class MovingWindowCrop:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        crop_length = min(self.crop_length, x.shape[1])
        start_idx = np.arange(0, x.shape[1] - crop_length + 1, max(1, self.crop_stride))
        return np.stack([x[:, i:i + crop_length] for i in start_idx], axis=0)
...
class SOSFilter:
    ...
    def __call__(self, x):
        if x is None or x.size == 0:
            raise ValueError("Input data for filtering is invalid or empty.")

        if x.shape[-1] < 15:  # Ensure length is greater than padlen (e.g., 15)
            print(f"Warning: Input sequence length {x.shape[-1]} is too short for filtering.")
            return x

        if x.ndim == 1:
            x = x.reshape(1, -1)

        filtered_data = np.empty_like(x)
        for i in range(x.shape[0]):
            filtered_data[i] = sosfiltfilt(self.sos, x[i])
...
class Standardize:
    ...
    def __init__(self, axis=None):
        if isinstance(axis, list):
            axis = tuple(axis)  # Convert list to tuple if provided as a list
        self.axis = axis

    def __call__(self, x):
        # Convert input to float for numerical stability
        x = x.astype(np.float32)

        # Ensure x has valid dimensions
        if x.ndim == 0:
            raise ValueError("Input data is empty or not valid.")
