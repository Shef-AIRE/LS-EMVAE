...
def evaluate(model: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        # Extract positive class probability if outputs have 2 dimensions (binary case)
        if outputs.size(1) == 2:
            outputs = outputs[:, 1]  # Keep only the probability for the positive class
    ...
    # Print metrics with the global average loss
