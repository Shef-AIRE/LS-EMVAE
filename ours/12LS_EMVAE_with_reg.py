...
def cross_modality_regularization(mu_joint, mus):
    return sum(nnf.mse_loss(mu, mu_joint) for mu in mus) / len(mus)
...
def train(epoch, model, dataloader, optimizer, annealing_epochs, accumulation_steps=2, log_interval=10):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        with autocast():
            ...
            loss = elbo_loss(px_z_list, lead_inputs, mu_joint, logvar_joint, batch_size, annealing_factor=annealing_factor)
...
model = LSEMVAE(prior_dist, latent_dim=n_latents, num_leads=12, input_dim_per_lead=5000)
...
optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-5)  # AdamW improves generalization
...
for epoch in range(1, epochs + 1):
    ...
    if train_loss < best_loss:
        ...
        torch.save(model.state_dict(), '/mnt/parscratch/users/ac1xms/mimic-iv-ecg-diagnostic-electrocardiogram-matched-subset-1.0/data_feature/pretrain/best_LS_EMVAE_with_reg_12_lead.pth')
        print(f"Saved best model with Loss: {best_loss:.6f}")

# Save final model
torch.save(model.state_dict(), '/mnt/parscratch/users/ac1xms/mimic-iv-ecg-diagnostic-electrocardiogram-matched-subset-1.0/data_feature/pretrain/LS_EMVAE_with_reg_12_lead.pth')
